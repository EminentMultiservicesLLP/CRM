...
{  
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model)
         {
            if (ModelState.IsValid)
            {
                ...
                model = model.GetUserIdNew(model.Username, model.Password); 
                Session["AppUserId"] = model.UserID;
                ...
                //else if (model.IsDeactive) {

                //    ViewBag.LoginFailedMsg = model.RoleId==7 ? model.DeactiveReason : "Login Failed, User is deactive";                    
                //    return View();
                //}
                else
                {
                    ...
                    FormsAuthentication.SetAuthCookie(model.Username, isRemember);
                    //if (model.RoleId == 3 || model.RoleId == 2 || model.RoleId == 5)
                    //{
                    if (model.LoginName == model.Password)
                    {
                        System.Web.HttpContext.Current.Session.Clear();
                        return RedirectToAction("FirstLogin", "LoginPage", new { username = model.LoginName, UserID = model.UserID });
                    }
                    //}
        ...
        {
            Logger.LogInfo("Loggin off to User:" + System.Web.HttpContext.Current.Session["AppUserId"].ToString());
